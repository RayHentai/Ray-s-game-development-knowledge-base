...
List\<T>:强在"按位置访问"和"有顺序、可遍历"。适合——数据有明确顺序、你常常"要第 N 个"或"从头到尾遍历"、主要在末尾增删。例:一个敌人列表按生成顺序排列、每帧遍历更新;背包格子按位置排列。
...
扩容的真实动作是:**新开一个两倍大的数组 → 把旧数组元素逐个拷贝过去 → 旧数组丢弃(等 GC)**。

如果事先知道大概要存多少,`new List<T>(容量)` 预设容量能避免反复扩容拷贝。
