...
所以UNIX时间戳是以1970年1月1日0点为计时起点时间的
...
//字符串输出
print(nowTime.ToString());//月/日/年 时:分:秒
print(nowTime.ToShortTimeString());//时:分
print(nowTime.ToShortDateString());//月/日/年
print(nowTime.ToLongTimeString());//时:分:秒
print(nowTime.ToLongDateString());//xx年xx月xx日

print(nowTime.ToString("D"));//LongDate
print(nowTime.ToString("yyyy-MM-dd-ddd/HH-mm-ss"));//自定义拼接打印
...
print(ts.TotalMinutes);//间隔分钟
print(ts.TotalSeconds);//间隔秒
print(ts.TotalDays);//间隔天
print(ts.TotalHours);//间隔小时
print(ts.Ticks);//纳秒间隔数

print(ts.Days + "-" + ts.Hours + "-" + ts.Minutes + "-" + ts.Seconds + "-" + ts.Milliseconds);//具体差了多少

//初始化它来代表时间间隔
TimeSpan ts2 = new TimeSpan(1,0,0,0);//不能用只有一个参数的重载 传入的参数代表Ticks
